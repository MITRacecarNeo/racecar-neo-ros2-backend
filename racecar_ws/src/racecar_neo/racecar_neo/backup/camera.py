    ...
    try:
        while rclpy.ok():        
            ...
            if got_frame:
                ...
                # End timer
                end_time = node.get_clock().now().to_msg()

                # Show time delta needed to process and send image
                start_time = start_time.sec + start_time.nanosec * 1e-9
                end_time = end_time.sec + end_time.nanosec * 1e-9
                time_delta = end_time - start_time
                # node.get_logger().info(f'Time Delta: {round(time_delta * 1e3, 3)} ms') # ~10-30ms
